...
public class AIAgent : Agent
{
    ...
    public Card lastPlayedCard; // Для отслеживания своей карты
    ...
    public override void CollectObservations(VectorSensor sensor)
    {
        ...
        // Информация о руке атакуемого игрока
        ...
        // Вероятность вытянуть взрывного котенка (есть взрывной котенок или нет, если нет, тогда вычисляем вероятность вытянуть котенка)
        ...
        // Информация о количестве ходов, которые нужно сделать
    ...
    public override void OnActionReceived(ActionBuffers actions)
    {
        ...
        // Разные ветки действий в зависимости от состояния игры
    ...
    private void AddRewardForPlayCard(bool playCard = true)
    {
        if (playCard)
        {
            AddReward(MathF.Pow(40 - gameManager.deck.cards.Count, 1.5f) * 0.02f);
    ...
    // ============ Обработка действий для разных состояний ============
    private void HandlePlayerTurnActions(ActionBuffers actions)
    {
        int actionType = actions.DiscreteActions[0];
        int behaviour = actions.DiscreteActions[1]; // 0 - защита; 1 - агрессия
    ...
    private void HandleNopeAwaitingActions(ActionBuffers actions, Card nopeCard)
    {
        ...
        if (useNope == 1)
        {
            // Проверяем, не пытается ли агент отменить свою же карту
            if (gameManager.playedCards.cards.Contains(lastPlayedCard))
            {
                // Штраф за отмену своей карты
                AddReward(-1f);
                Debug.LogWarning($"{player.name}: Штраф! Попытка отменить свою карту {lastPlayedCard.cardName}");
            }
            else
            {
                ...
                Debug.Log($"{player.name}: Использует Nope против {lastCard.name}");
            }
        ...
        {
            // Воздержаться от использования Nope
            AddReward(0.1f); // небольшое поощрение за пассивность
    ...
    private IEnumerator TakeCardCoroutine()
    {
        if (seeTheFutureCards.Count > 0)
        {
            if (seeTheFutureCards[0].IsExplodingKitten() && gameManager.deck.cards.Count > 1)
            {
                if (playableCards.Count > 0)
                {
                    Debug.LogWarning($"{player.name} взял взрывного котенка зная о нем!");
                    AddReward(-5f);
                }
    ...
    private IEnumerator PlayCardCoroutine(Card card)
    {
        ...
        if (card.IsCatCard() || card.IsCardNameByItsName(Card.CardName.Favor))
        {
            ...
            {
                Debug.LogWarning($"{name} неправильно использовал карту {card.name} на пустую руку {attackedPlayer.name}");
                AddReward(-5f);
                actionInProgress = false;
                yield break;
            }
    ...
    private IEnumerator GiveFavorCardCoroutine(Card cardToGive)
    {
        ...
        Debug.Log($"{player.name} отдает карту {cardToGive.cardName} по требованию Favor");
        actionsCount++;
        actionInProgress = false;
    }
    ...
    // ============ Методы для получения наград из GameManager ============
